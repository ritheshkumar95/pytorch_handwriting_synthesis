...
from modules import HandwritingPredictionNetwork
...
def parse_args():
    ...
    parser.add_argument("--path", default='./my_data')
...
def monitor_samples():
    ...
    for i in range(8):
        ...
        with torch.no_grad():
            stroke_loss, eos_loss, _, teacher_forced_sample = model.compute_loss(
                strokes, strokes_mask
            )
            generated_sample = model.sample(batch_size=1)
...
def train(epoch):
    ...
    for iterno, data in enumerate(train_loader):
        ...
        for idx in range(1, seq_len, args.seq_len):
            stroke_loss, eos_loss, prev_states, _ = model.compute_loss(
...
def test(epoch):
    ...
    for iterno, data in enumerate(test_loader):
        ...
        with torch.no_grad():
            stroke_loss, eos_loss, _, _ = model.compute_loss(
                strokes, strokes_mask
            )
