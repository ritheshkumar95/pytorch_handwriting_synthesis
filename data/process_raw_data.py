...
def get_stroke_sequence(filename):
    ...
    coords = utils.align(coords)
    coords = utils.denoise(coords)
    offsets = utils.coords_to_offsets(coords)
    offsets = offsets[:utils.MAX_STROKE_LEN]
    offsets = utils.normalize(offsets)
    return offsets
...
def get_ascii_sequences(filename):
    ...
    lines = [utils.encode_ascii(line)[:utils.MAX_CHAR_LEN] for line in lines]
    return lines
...
if __name__ == '__main__':
    print('downloading blacklist file...')
    os.system(
        'wget https://github.com/sjvasquez/handwriting-synthesis/raw/master/data/blacklist.npy -P ./data'
    )
    print('traversing data directory...')
    stroke_fnames, transcriptions, writer_ids = collect_data()

    print('dumping to numpy arrays...')
    x = np.zeros([len(stroke_fnames), utils.MAX_STROKE_LEN, 3], dtype=np.float32)
    x_len = np.zeros([len(stroke_fnames)], dtype=np.int16)
    c = np.zeros([len(stroke_fnames), utils.MAX_CHAR_LEN], dtype=np.int8)
